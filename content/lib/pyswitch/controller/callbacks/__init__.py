...
class BinaryParameterCallback(Callback):
    ...
    def evaluate_value(self, value):
        # Local stomp switches must retain their state when a device does not
        # echo the CC message back immediately (or is temporarily unplugged).
        if value is None and self.__use_internal_state:
            return
   ...
   # Update label color, if any
    def set_label_color(self, color):
        ...
        label_color = dim_color(color, self.__display_dim_factor_on if self.action.state == True and (self.mapping.response or self.__use_internal_state) else self.__display_dim_factor_off)

        if self.action.label.back_color is None:
            self.action.label.text_color = label_color
        else:
            self.action.label.back_color = label_color
