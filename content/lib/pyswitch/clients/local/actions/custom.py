...
def CUSTOM_MESSAGE(message,                 # Raw MIDI message bytes (as list, for example [176, 80, 0] for a control change). You can use hex values in format 0xab, too.
                   ...
                   display = None,
                   text_color_only = False, # Keep low-memory displays free of background rectangles.
                   use_leds = True,
                   id = None,
    ...
    return Action({
        "callback": _CustomMessageCallback(
            ...
            text = text,
            text_color_only = text_color_only
        ),
        "display": display,
...
class _CustomMessageCallback(Callback):
    ...
    def __init__(self, 
        ...
        self.__text_color_only = text_color_only
    ...
    def update_displays(self):
        ...
        if self.action.label:
            ...
            if self.__text_color_only:
                self.action.label.text_color = self.__color
            else:
