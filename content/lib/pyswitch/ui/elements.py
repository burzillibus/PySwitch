...
class DisplayLabel(DisplayElement):
    ...
    def init(self, ui, appl):
        ...
        if self.__layout.back_color and not get_option(getattr(appl, "config", {}), "disableDisplayBackgrounds", False):
            collect()
    ...
    def __update_font(self):
        font_loader = getattr(self.__ui, "font_loader", None)
        if not font_loader and self.__appl:
            font_loader = getattr(self.__appl, "font_loader", None)

        self.__font = font_loader.get(self.__layout.font_path)
    ...
    def back_color(self, color):
        disable_backgrounds = bool(self.__appl and hasattr(self.__appl, "config") and get_option(self.__appl.config, "disableDisplayBackgrounds", False))
        ...
        if disable_backgrounds:
            ...
            # Keep labels readable when background rectangles are disabled.
