...
**CircuitPython version:** The original bundled `.mpy` libraries were built for CircuitPython 8 and are incompatible with CircuitPython 9 or 10. The current bundle is built for CircuitPython 10; use CircuitPython 10.x on the controller and do not mix in libraries built for CircuitPython 8 or earlier.
...
#### BOSS GX-100 MIDI input

The GX-100 can send a Program Change when its **MEMORY MIDI** feature is enabled, and Control Changes configured through its **ASSIGN** settings. Route the USB or DIN input to `MidiController.APPLICATION` as shown above, and set the GX-100 TX channel to the device `in_channel` (or use `in_channel = None`).

Use the GX-100 mappings in callbacks or display elements:

```python
from pyswitch.clients.boss.gx100.mappings import MAPPING_RX_MEMORY_CHANGE
from pyswitch.controller.callbacks.parameter_display import ParameterDisplayCallback

# The received, zero-based Program Change number.
memory_callback = ParameterDisplayCallback(MAPPING_RX_MEMORY_CHANGE())
```

Use `MAPPING_RX_ASSIGN_CONTROL_CHANGE(12)` in the same way for a GX-100 ASSIGN that transmits CC#12. The mappings only receive GX-100 input; generic PySwitch mappings already send Program Changes and Control Changes to the GX-100.
